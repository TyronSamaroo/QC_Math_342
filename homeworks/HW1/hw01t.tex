...
Bias tries to oversimplifies things and miss the important parts leading to high errors. Variance tries to over over compensate and fail to generalize the data, it performs well on testing set but not on real data. Silver term precision seems to be connect to variance and accuracy to bias.  
...
The domain should be the inputs from data \mathbb{D} and the set of \matcal{H}. Its range should be the output of the approximation of the phenomenon $y$

\easysubproblem{Is $g \in \mathcal{H}$? Why or why not?}\spc{3}

$g \in \mathcal{H}$ because we defined an algorithm \mathcal{A} which produces $g$ when given data $\mathbb{D}$ and $\matcal{H}$ 
...
$y = \underbrace{g(x)}_{model} + \underbrace{
                        \underbrace{(h^*(x) - g(x))}
                        _{estimation error} + 
                        \underbrace{
                        \underbrace{(f(x) - h^*(x))}_{missspecification error} + 
                        \underbrace{(t(z) - f(x))}
                        _{\delta}
                        }_{\mathcal{E}}
                        }_{e}$

1. $\delta$, ignorance error, not enough features that contain useful information.

2. Misspecificaiton Error, This happens when model is too simple and        hypothesis set $\mathcal{H}$ needs to be expanded to make more complicated functions

3. Estimation Error, Sample size is too small, now enough data 
...
1. $\delta$, ignorance error, measure more features

2. Misspecificaiton Error, Expand hypothesis set $\mathcal{H}$ to make more complicated functions

3. Estimation Error, Increase Sample size by collecting more data
