...
Because children tend to move toward the mean
...
Everyone calls it regression,because of Galton discovery of Parents and Child height moving towards the mean. 

Create a dataset $\mathbb{D}$ which we call `Xy` such that the linear model as $R^2$ about 50\% and RMSE approximately 1.

```{r}
x = c(1,2,3,4,5,6,4,1,2,3)
y = c(1,4,3,6,4,7,3,2,3,3)
Xy = data.frame(x = x, y = y)
first_model = lm(y~ x)
summary(first_model)$r.sq
summary(first_model)$sigma
```
...
Xy = data.frame(x = x, y = y)
sec_model = lm(y~ x)
summary(sec_model)$r.sq
summary(sec_model)$sigma
```
...
x = iris$Species
y = iris$Petal.Length
petallen_model = lm(y ~ x)
summary(petallen_model)$r.sq
summary(petallen_model)$sigma
