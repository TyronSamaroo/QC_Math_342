...
```{r}
mode = function(x){
  ifelse(length(x[x == 0]) > length(x[x == 1]), 0, 1)
}
y
...
#' percentron_learning_algorithm
#' TO-DO: Explain what this function does in a few sentences
#' This function takes input data X and y labeled data y and some weight w that will 
#'
#' @param Xinput      Matrix with features and each observations
#' @param y_binary    Label data that we are trying to predict 
#' @param MAX_ITER    Max steps the algorithm will run 
#' @param w           Weights
#'
...
perceptron_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 1000, w = NULL){
  ...
  n = nrow(Xinput)
  p = ncol(Xinput) # features 
  #num_errors = array(NA, length(Xinput))
...
```{r}
pacman::p_load(e1071)
svm_model = svm(X_simple_feature_matrix, 
                Xy_simple$response,
                kernel = 'linear',
                scale= NULL)
```
...
linear_svm_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 5000, lambda = 0.1){
  ...
  # SHE = max(0, 0.5 - (y_i - 0.5) * (w %*% x_i - b)  / n
  # min( 1/n * SHE + lambda length(w) ^2)
  #
  #
}
```
