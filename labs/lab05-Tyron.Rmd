...
```{r}
H_comp = diag(nrow(X)) - H
rankMatrix(H_comp, tol = 1e-2)
```
...
Trace is equal to rank

Find the eigendecomposition of both `H` and `Hcomp` as `eigenvals_H`, `eigenvals_H`, `eigenvals_Hcomp`, `eigenvecs_Hcomp`. Verify these results are the correct dimensions.

```{r}
eigen_H = eigen(H)
eigen_Hcomp = eigen(H_comp)
...
```{r}
eigenvals_H = round(Re(as.numeric(eigenvals_H)),10)
eigenvecs_H = round(Re(as.numeric(eigenvecs_H)),10)

eigenvals_Hcomp = round(Re(as.numeric(eigenvals_Hcomp)),10)
eigenvecs_Hcomp = round(Re(as.numeric(eigenvecs_Hcomp)),10)
...
round(apply(eigenvecs_H, FUN = function(v){ sqrt(sum(v^2))}),10)
...
No, the eigen vectors are sacled to be unit length
