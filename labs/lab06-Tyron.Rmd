...
in_sample_model = lm(y_train ~ X_train)
in_sample_error = sd(in_sample_model$residuals)
...
while(itter < 1000){
  ...
  in_sample_model = lm(y_train ~ X_train)
  in_sample_error = in_sample_error + sd(in_sample_model$residuals)
  
  out_sample_y_hat = predict(in_sample_model, data.frame(X_test))
  out_sample_residuals = y_test - out_sample_y_hat
  out_sample_error = out_sample_error + sd(out_sample_residuals)
...
for(i in 1:n){
  ...
  s_es[i] = sd(summary(insample_model$residuals))
}
s_es
plot(1:n, s_es)
```
...
oos_es = array(NA,nrow(Xjunk))

for(i in 1:n){
  model = lm(y_train ~ X_train[,1:i])
  oos_y_hat = predict(model, data.frame(X_test))
  oos_residuals = y_test - oos_y_hat
  oos_es[i] = sd(oos_residuals)
  print(i)
